...
def build_silver_taxi_zones(bronze: DataFrame) -> DataFrame:
    ...
    def has_control_chars(column_name: str):
        return F.coalesce(F.col(column_name).rlike(r"[\x00-\x1F\x7F]"), F.lit(False))

    standardized = bronze.select(
    ...
    assessed = (
        ...
        .agg(
            F.sort_array(
                F.collect_set(F.struct("borough", "zone", "service_zone"))
        ...
        .withColumn("variant_count", F.size("_business_variants"))
        .withColumn(
            "duplicate_row_count",
            F.col("source_row_count") - F.col("variant_count"),
        )
    )
    canonical_variant = F.element_at("_business_variants", 1)
    return (
        assessed.select(
            "location_id",
            F.when(F.col("variant_count") == 1, canonical_variant.borough)
            .otherwise(F.lit(None).cast("string"))
            .alias("borough"),
            F.when(F.col("variant_count") == 1, canonical_variant.zone)
            .otherwise(F.lit(None).cast("string"))
            .alias("zone"),
            F.when(F.col("variant_count") == 1, canonical_variant.service_zone)
            .otherwise(F.lit(None).cast("string"))
            .alias("service_zone"),
        ...
        .withColumn("has_duplicate_key", F.col("source_row_count") > 1)
        .withColumn("has_conflicting_values", F.col("variant_count") > 1)
        .withColumn(
        ...
        .withColumn(
            ...
            .when(F.col("has_duplicate_key"), F.lit("DUPLICATE"))
            .otherwise(F.lit("OK")),
        )
    )
