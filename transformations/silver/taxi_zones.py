...
CATALOG = spark.conf.get(
    "nyc_mobility.catalog",
    "nyc_mobility",
)

BRONZE_SCHEMA = spark.conf.get(
    "nyc_mobility.bronze_schema",
    "nyc_bronze",
)

SILVER_SCHEMA = spark.conf.get(
    "nyc_mobility.silver_schema",
    "nyc_silver",
)


BRONZE_TAXI_ZONES = (
    f"{CATALOG}.{BRONZE_SCHEMA}.bronze_taxi_zones_raw"
)

SILVER_TAXI_ZONES = (
    f"{CATALOG}.{SILVER_SCHEMA}.silver_taxi_zones"
)
...
def silver_taxi_zones():
    ...
    latest_rows = (
        ...
        .filter(
            F.col("_ingested_at").eqNullSafe(
                F.col("_latest_ingested_at")
            )
        )
    ...
    assessed = (
        ...
        .agg(
            ...
            F.max(
                F.col("had_control_char").cast("int")
            ).alias("_had_control_char"),
        )
        .withColumn(
            "variant_count",
